...
test<-fread("./project/volume/data/interim/house_test.csv")
train<-fread("./project/volume/data/interim/house_train.csv")
...
train_pred<-predict(XGBm, newdata = dtrain)


#check error using train data
rmse(y.train,train_pred)

house_submit$SalePrice<-pred

#now we can write out a submission
fwrite(house_submit,"./project/volume/data/processed/house_v4.csv")
