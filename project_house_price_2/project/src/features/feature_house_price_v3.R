...
master$qualNum <- as.numeric(gsub("\\D","",master$Qual))
master$condlNum <- as.numeric(gsub("\\D","",master$Cond))

master$qualLetter <- gsub("\\d","",master$Qual)
master$condLetter <- gsub("\\d","",master$Qual)
